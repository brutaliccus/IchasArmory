...
- Community browser: authenticated **Save** publishes to `data/community-gear-plans/`; **Share** writes a `gear-plans/` snapshot for `?gp=` only (`community: false`) and is never listed. Browse Builds fetches the full catalog (`GET /community-gear-plans?all=1`, no silent 5000 ceiling; index reconciled from plan files and logged-in Save publishes, not share snapshots) then search/filters that list and paginates matches with Previous / page numbers / Next (50 per page). Role aliases and talent-based spec/role inference keep older healer rows visible. Save requires role + talent-tree `spec` + icon (`data/wow-icons.json`).
