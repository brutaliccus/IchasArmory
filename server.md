...
- **GET /community-gear-plans** — Public list/search (`q`, `class`, `role`, `spec`, `sort=popular|recent`, `voterId`, `limit`, `offset`). Default sort **Popular** (`score = upvotes - downvotes`). Filters apply to the full community index before pagination; response includes `plans`, `total`, `hasMore`, `offset`, `limit` (default `limit=50`, max 100). Returns card metadata including `description`, `icon`, talentSpread, votes.
