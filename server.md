...
- **POST /gear-plans** — Share snapshot only. Writes `gear-plans/{id}.json` with `community: false` and returns `{ planId }` for `?gp=`. Does **not** add the plan to the community index. Opening `?gp=` loads view mode and does not publish.
- **GET /community-gear-plans** — Public list/search (`q`, `class`, `role`, `spec`, `sort=popular|recent`, `voterId`, `limit`, `offset`, `all=1`). Default sort **Popular** (`score = upvotes - downvotes`). Each request reconciles `index.json` from on-disk `{id}.json` files and logged-in **Save** publishes (`user.gearPlans` with `community !== false`). Share snapshots in `gear-plans/` are never imported. Leftover share-only community copies (`local_gp_*` or 8-char share codes that are not a user’s published save) are dropped from the listing. Role aliases (`heal`/`healing` → `healer`); missing spec/role are inferred from talent trees so older healer rows are not dropped. Filters apply to that full index before paging; `q` matches name, author, description, spec, and class. Response includes `plans`, `total`, `hasMore`, `offset`, `limit`. Omitted `limit` or `all=1` / `limit=all` returns the **full** filtered set (no 5000 ceiling). Explicit `limit` is clamped to 1–500 for paged fetches (`hasMore` stays accurate so clients can walk the rest). Returns card metadata including `description`, `icon`, talentSpread, votes.
