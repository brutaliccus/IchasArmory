...
`missChance` is exported on the return object as **Miss** in UI (nested under **Avoidance**). `totalMitigation` is shown as **Avoidance** (dodge + parry + block + miss). Character Planner and Gear Planner **Defense** columns also show **Defense** (skill rating), **Effective HP** (`ehp`), **DR** (`physicalDR`), **Mit Score**, and **Tank Score** (EHP from stamina/armor weights + mitigation score; requires tank sim stat weights).
