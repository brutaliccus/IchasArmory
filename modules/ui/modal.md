...
- **Match logic:** handedness is OR among selected One-Handed/Two-Handed/Shield; subtype is OR among selected types; the two groups AND together. Empty handedness (or all visible hands) is unrestricted so **Sword** matches both `Main Hand`+`Sword` and `Two-hand`+`Sword`. Checking **Two-Handed** further restricts. List rows show `formatWeaponTypeLabel` (e.g. `Two-Hand • Sword`). Real stats still AND. Armor types stay OR among Plate/Mail/Leather/Cloth and are not affected.
