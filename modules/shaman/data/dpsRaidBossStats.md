...
- **Journal portraits:** Where encounter journal art exists on octowow, URLs use  
...
- **Bulk journal icons:** `npm run gen:dps-boss-icons:zam` runs **`scripts/apply-zam-ej-icons-to-dps-boss-json.py`**, which HEAD/GET-checks each boss in `raidDefinitions` and sets **`iconUrl`** on octowow when the CDN returns 200.
