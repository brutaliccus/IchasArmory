...
- **Community browser:** `#gp-community-search-btn` opens `#gp-community-search-dialog` — search name/author; filters class/role/spec; sort Popular (default) / Recent. Result **cards** show icon, talent spread `0/0/0`, title, class/role/spec, author, upvote/downvote (stopPropagation). Card click loads via `setGearPlan` + `ensureGearPlanItemsReady`. Guests can browse and vote (anon localStorage voter id).
