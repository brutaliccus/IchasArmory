...
- Gear Planner class/race/talents/**buffs** are **independent** of the Character Planner. **Talents** and **Buffs** are full views (not modals): they hide slot columns. Opening snapshots Character Planner trees/buffs (moves `#buffs-list` + consume presets into GP) and restores on close. The active view’s header button morphs into the Gear Planner (armor) icon to return to slots — no Done button. Talent host is `zoom: 2`.
