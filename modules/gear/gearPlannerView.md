...
- `#gp-stats-sidebar` … Modified stats Melee group includes weapon skill (or dual-wield MH/OH skill), with **Dodge** and **Glancing** nested under weapon skill (`.gp-stat-item--nested`), plus AP-vs-creature rows (merged into Melee, not a separate category).
...
- Talents view: `#gp-talents-header` row — **`#gp-talents-title`** (plan class name) left, `#gp-talent-preset-tools` hamburger right (**shaman only**, hidden for all other classes); `syncGpTalentsHeaderLayout()` / `fitGpTalentTree()` align the header over the scaled tree block (width + `margin-left` from tree `getBoundingClientRect()`, title font size = `.tree-name` font × tree scale); talent tree is **top-aligned** in `#gp-talents-host` (scale transform from top center).
