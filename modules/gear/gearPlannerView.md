...
- `#gear-planner-shell` is full-width flex, centered; `.gp-main` holds the canonical center column. The locations/stats docks overlay the screen edges and do **not** pad the shell by sidebar width.
...
- **Community browser:** cards show icon, description, talent spread, role labels (DPS/Tank/Healer), SVG-only up/down votes (green/red when active) with counts, and **Favorite** (copies into My Gear Plans; idempotent via `sourceCommunityId`). Header votes sit immediately after `#gp-plan-name` in `.gp-plan-title-row` when a community plan is open (own, shared community id, or favorite via `sourceCommunityId`); voting uses `getCommunityVoteId()` and `hydrateCommunityVoteMeta()` on load.
...
- Talents view: `#gp-talent-preset-tools` hamburger (shaman only) applies `SHAMAN_TALENT_PRESETS`; talent tree scales via `transform` to fit `#gp-talents-host` within the fixed center column (no shell width change).
