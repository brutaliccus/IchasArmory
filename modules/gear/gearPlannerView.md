...
- Save: always opens `#gp-save-overwrite-dialog` (item-picker chrome: backdrop + `item-picker-panel`, custom `stat-dropdown` selects for **role** and **spec**, `item-picker-search` fields). Footer is one row (`justify-content: space-between`): left **Overwrite** + **Save as New** when the plan already exists; right **Cancel** + **Save** icon buttons only (`gp-btn-icon`). Warns (confirm) if missing talents / buffs / role-appropriate stat weights. Cloud saves publish to community; local guest saves do not.
...
- **Stat weights:** Dual-role plans show **Tank** and **Single Target (DPS)** panels side by side (`gp-dual-weights`); shaman AoE panel is hidden in dual layout. GP stat weight tables include `.sort-indicator` spans for sort UI. Generate re-queries live DOM after `renderGearPlanner()` so progress/button updates do not throw on detached nodes.
...
- `#gp-quick-sim-btn`: Shaman-only header icon (sword SVG, `.gp-btn-icon`, same size as other GP header icons). Result text in `#gp-quick-sim-result`. `#gp-sim-settings-btn`: shaman-only cog (also `.dps-sim-config-open-btn`) opens `#dps-sim-config-modal` via `openDpsSimConfigModal()`; `prepareDpsSimConfigForGearPlanner()` bootstraps the modal on GP init/class change when the plan is shaman. Modal must be portaled to `document.body` with `data-ichacalc-sim-config-ready` (see `isDpsSimConfigModalReady` in `dps.js`).
