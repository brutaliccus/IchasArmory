...
- Talents view: `#gp-talents-header` row — **`#gp-talents-title`** (plan class name) left, `#gp-talent-preset-tools` hamburger right (**shaman only**, hidden for all other classes); header width and title font size sync in `syncGpTalentsHeaderLayout()` / `fitGpTalentTree()` to match scaled `.tree-name`; talent tree is **top-aligned** in `#gp-talents-host` (scale transform from top center).
