...
- `data/loot/item-sources-lite.json` — compact `[instanceId, instanceName, kind]` tuples used at runtime
...
| Function | Purpose |
|----------|---------|
| `ensureItemSourcesLoaded()` | Fetch instances index + lite sources once (`cache: 'force-cache'`); falls back to full JSON |
